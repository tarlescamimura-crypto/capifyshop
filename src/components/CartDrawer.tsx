    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        ...
        >
          ...
          {totalItems > 0 && (
            <Badge className="absolute -top-2 -right-2 h-5 min-w-5 rounded-full p-0 flex items-center justify-center text-xs bg-primary text-primary-foreground border-2 border-background">
              {totalItems}
            </Badge>
          )}
        </Button>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Comece a adicionar produtos</p>
              </div>
            </div>
          ) : (
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0 space-y-4">
                {items.map((item) => (
                  <div
                    key={item.variantId}
                    className="flex gap-4 p-3 rounded-lg bg-background/50 border border-border"
                  >
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border bg-card">
                ...
                <p className="text-xs text-center text-muted-foreground">
                  Checkout 100% seguro via Shopify
                </p>
              </div>
            </>
          )}
